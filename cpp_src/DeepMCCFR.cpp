...
DeepMCCFR::~DeepMCCFR() {
    // Этот вызов остаётся как дополнительная мера предосторожности,
    // но основной сброс теперь происходит в run_traversal_loop.
...
void DeepMCCFR::run_traversal_loop() {
    GameState state; 
    while (!stop_flag_->load(std::memory_order_relaxed)) { // Используем relaxed, т.к. нам не нужны строгие гарантии синхронизации с другими данными
        traverse(state, 0);
        state.reset(); 
        traverse(state, 1);
        state.reset();
    ...
    // --- ГЛАВНОЕ ИСПРАВЛЕНИЕ ---
    // Гарантированно сбрасываем всё, что осталось в локальном буфере,
    // после того как получили сигнал на остановку и вышли из цикла.
    // Это предотвращает потерю последних сгенерированных сэмплов.
    flush_local_buffer();
    // --- КОНЕЦ ИСПРАВЛЕНИЯ ---
}
